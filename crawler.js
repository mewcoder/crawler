  ...
  callback: (error, res, done) => {
    ...
    //  console.log(date);
     ...
     // $content.find('img').remove();
      ...
      $content.find('.confluence-embedded-file-wrapper').remove();
...
crawlerMeta.on('drain', () => {
  ...
  articlesLink.forEach((link) => {
    let URL='https://wiki.hikvision.com.cn/login.action?os_destination=%2Fpages%2Fviewpage.action%3FpageId%3D'+link.substring(link.length-9)+'&permissionViolation=true';
    let uriObject = {
      uri : URL,
      headers : {
        'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.75 Safari/537.36',
...
// const res ={};
// async (res) => {
//   res =  search.queryCount();
 
// }
function up(date) {
    const rep = search.index(date);
    console.log(rep);
}
console.log('=========================ES end=============================');
